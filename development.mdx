...
Follow these steps to install and run the documentation development server on your operating system.

<Steps>
<Step title="Install the Documentation CLI">
...
By default, the documentation server uses port 3000. You can customize the port by using the `--port` flag. For example, to run on port 3333, use this command:
...
Please note that each CLI release is associated with a specific version of the documentation system. If your local preview does not align with the production version, please update the CLI:
