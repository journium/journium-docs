...
Markov chain models have a long history in modeling sequential user behavior. Charan \cite{charan2015} applies Markov chains to customer journey attribution across marketing touchpoints. Harbich et al.\ \cite{harbich2017} propose a mixture of Markov models for customer journey map discovery. The MDPI Customer Behaviour Hidden Markov Model \cite{wang2022} extends this to e-commerce with hidden state representations.
...
\subsection{Layer 3: Behavioral Knowledge Graph}
...
\subsection{Layer 3: Detector System}
...
The weights $(\alpha, \beta, \gamma, \delta, \varepsilon)$ are configurable per deployed monitoring context. We acknowledge that the definition of interestingness is inherently subjective \cite{silberschatz1995}.
...
\subsubsection{Unexpected Loop Detector}

The loop detector identifies states visited repeatedly within a single journey, indicating user confusion, friction, or a broken flow. A state $s$ is flagged as a loop when the mean visit count among journeys containing $s$ exceeds a threshold $\tau_{\text{loop}}$.
...
High-quality paths are surfaced as findings with predicate \texttt{is\_fast\_path\_to}. Low-quality paths with high occurrence rates are flagged as candidate optimization targets.
...
For a given finding $F$, the fact bundle $B(F)$ is constructed by retrieving from the BKG all facts that: (a) have \texttt{relatedEntityIds} intersecting the finding's entity set, (b) are valid within the finding's time window, and (c) exceed the minimum confidence threshold. Bundle construction uses a bounded-depth graph traversal (one-hop in v1). This produces a structured context document such as:
...
For the GLL, faithfulness is evaluated by extracting verifiable claims from the narrative and checking each against the fact bundle, analogous to the claim-verification methodology of InsightBench \cite{srinivasan2024}.
...
\bibitem{bernard2017}
Bernard, G. and Andritsos, P. (2017).
CJM-Miner: Mining Customer Journey Models from Customer Behavioral Data.
\textit{EDBT}.

\bibitem{harbich2017}
Harbich, M., Bernard, G., Berkes, P., Garbinato, B., and Andritsos, P. (2017).
Discovering Customer Journey Maps using a Mixture of Markov Models.
\textit{International Symposium on Data-Driven Process Discovery and Analysis}.

\bibitem{he2024}
He, X., Zhou, M., et al.\ (2024).
...
\bibitem{qian2023}
Qian, Y., Ding, B., and Zhou, J. (2023).
InsightPilot: An LLM-Empowered Automated Data Exploration System.
\textit{EMNLP Demos 2023}.

\bibitem{silberschatz1995}
Silberschatz, A. and Tuzhilin, A. (1995).
On Subjective Measures of Interestingness in Knowledge Discovery.
\textit{KDD 1995}.

\bibitem{srinivasan2024}
Srinivasan, A., et al.\ (2024).
InsightBench: Evaluating Business Analytics Agents through Multi-Step Insight Generation.
\textit{ICLR 2025}.

\bibitem{temouden2020}
Temouden, K. (2020).
Clustering Customer Journeys using a Mixture of Markov Models.
\textit{Engineered @ Publicis Sapient Blog}.

\bibitem{wang2022}
Wang, H. et al.\ (2022).
Customer Behaviour Hidden Markov Model.
\textit{Mathematics} 10(8):1230. MDPI.

\bibitem{xu2020}
Xu, K., Ma, X., and Zhang, D. (2020).
...
\bibitem{charan2015}
Charan, A. (2015).
Multi-Channel Attribution Modeling: The Mathematical Overview.
